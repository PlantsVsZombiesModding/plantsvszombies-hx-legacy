...
class SnowPea extends CPlant {
	...
	public function Fire(theTargetZombie:Zombie, theRow:Int, thePlantWeapon:Int) {
		...
		if (mSeedType == CPlant.SEED_PEASHOOTER || mSeedType == CPlant.SEED_SNOWPEA || mSeedType == CPlant.SEED_REPEATER) {
			this.GetPeaHeadOffset(aOffsetX, aOffsetY);
			aOriginX = mX + aOffsetX + 30;
			aOriginY = mY + aOffsetY + 5;
		}
		var anEffect = app.particleManager.spawnParticleSystem(PVZParticles.PARTICLE_SNOWPEAPUFF);

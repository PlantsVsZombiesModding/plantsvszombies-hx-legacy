...
class MainMenuState implements IState implements IButtonListener implements ICheckboxListener {
	...
	public var mShowingDialog:Bool = false;

	var menuReanim:Reanimation;

	var mUpsellButton:CButtonWidget;

	var Menu_MusicCheckBox:Int = 105;

	var mPuzzleButton:ImageButtonWidget;

	var app:PVZApp;

	var mAdventureButtonImage:ImageInst;

	var mMinigameButton:ImageButtonWidget;

	public var mAdventureLevel:Bool = false;

	var mAdventureButton:ImageButtonWidget;

	var GameSelector_Sound:Int = 104;

	var GameSelector_Adventure:Int = 100;

	var mStageImage:ImageInst;

	var GameSelector_Puzzle:Int = 102;

	var GameSelector_Minigame:Int = 101;

	var mStartingGame:Bool = false;

	var mUpsellCounter:Int = 0;

	public var mSoundCheckBox:CheckboxWidget;

	var mURLRequest:URLRequest;

	var mSoundButton:ImageButtonWidget;

	public var mDialogBox:DialogBox;

	var GameSelector_Upsell:Int = 105;

	public var mMusicCheckBox:CheckboxWidget;

	var mLevelImage:ImageInst;

	var mStartingGameCounter:Int = 0;

	var mStartingLevel:Float = 1;

	var GameSelector_Survival:Int = 103;

	var zombieHandReanim:Reanimation;

	var mSurvivalButton:ImageButtonWidget;

	var Menu_SoundCheckBox:Int = 104;

	var boardImg:ImageInst;

	var mAdventureButtonHighlight:ImageInst;
	...
	function initMenuReanim() {
		...
		this.menuReanim.overrideImage("SelectorScreen_ZenGarden_button",
			this.app.imageManager.getImageInst(PVZImages.IMAGE_REANIM_SELECTORSCREEN_VASEBREAKER_BUTTON_LOCKED));
		if (this.app.mPuzzleLocked) {
			this.menuReanim.overrideImage("SelectorScreen_Challenges_button",
				this.app.imageManager.getImageInst(PVZImages.IMAGE_REANIM_SELECTORSCREEN_CHALLENGES_BUTTON_LOCKED));
		}
		if (this.app.mSurvivalLocked) {
			this.menuReanim.overrideImage("SelectorScreen_Survival_button",
				this.app.imageManager.getImageInst(PVZImages.IMAGE_REANIM_SELECTORSCREEN_SURVIVAL_BUTTON_LOCKED));
		}
		if (this.app.mLevel == 1) {
			this.menuReanim.setTrackVisible("SelectorScreen_Adventure_button", false);
		...
		this.mUpsellButton.setColor(Std.int(CButtonWidget.COLOR_LABEL), Color.RGB(1, 1, 1));
		this.mUpsellButton.setColor(Std.int(CButtonWidget.COLOR_LABEL_HILITE), Color.RGB(179 / 255, 158 / 255, 110 / 255));
		var font5 = this.app.fontManager.getFontInst(PVZFonts.FONT_BRIANNETOD16);
		font5.scale = 0.9;
	...
	public function unlockAllModes() {
	...
	public function buttonRelease(id:Float) {
		...
		switch (id) {
			case(_ == this.GameSelector_Upsell => true):
			...
			case(_ == this.GameSelector_Adventure => true):
			...
			case(_ == this.GameSelector_Puzzle => true):
				if (this.app.mPuzzleLocked) {
			...
			case(_ == this.GameSelector_Survival => true):
				if (this.app.mSurvivalLocked) {
					this.mShowingDialog = true;
					this.ButtonsOff();
					this.mDialogBox.mDialogType = DialogBox.DIALOG_LOCKED;
			...
			case(_ == this.GameSelector_Minigame => true):
				if (this.app.mUpsellOn) {
					this.ButtonsOff();
					this.app.stateManager.pushState(PVZApp.STATE_UPSELL_SCREEN);
				} else {
	...
	public function GetLevelNumbers() {
		var img:ImageInst = null;
		var theLevel = this.app.mLevel;
		var aArea = TodCommon.ClampInt(Std.int((theLevel - 1) / Board.LEVELS_PER_AREA + 1), 1, 6);
		var aSubArea = theLevel - (aArea - 1) * Board.LEVELS_PER_AREA;
		...
		var digits = Utils.getDigits(aSubArea);
		var images = new Array<ASAny>();
		var srcRects = new Array<ASAny>();
		var destPoints = new Array<ASAny>();
	...
	public function MakeCheckBox(theImage:ImageInst):ImageInst {
		...
		var check = this.app.imageManager.getImageInst(PVZImages.IMAGE_SELECTORSCREEN_SOUNDX);
	...
	function initSoundCheckboxes(visible:Bool) {
		var checkbox = this.app.imageManager.getImageInst(PVZImages.IMAGE_SELECTORSCREEN_SOUND2);
		var emptyCheckbox = this.MakeCheckBox(this.app.imageManager.getImageInst(PVZImages.IMAGE_SELECTORSCREEN_SOUND1));

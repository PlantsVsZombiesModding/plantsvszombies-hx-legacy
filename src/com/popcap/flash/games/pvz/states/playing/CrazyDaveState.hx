...
class CrazyDaveState implements IState implements IButtonListener {
	...
	public function AdvanceCrazyDaveText():Bool {
		...
		if (aMessageName == "[CRAZY_DAVE_2416]" || aMessageName == "[CRAZY_DAVE_3003]" || aMessageName == "[CRAZY_DAVE_208]"
			|| aMessageName == "[CRAZY_DAVE_3303]") {
			this.CrazyDaveDie();
		} else {
	...
	public function UpdateCrazyDave() {
		...
		switch (this.mCrazyDaveReanim.currentTrack) {
			...
			case "anim_blahblah" | "anim_smalltalk" | "anim_mediumtalk" | "anim_crazy":
				this.mCrazyDaveState = CRAZY_DAVE_TALKING;
		...
		if (this.app.mBoard.IsWallnutBowlingLevel()
			&& this.app.mBoard.mTutorialState == Board.TUTORIAL_SHOVEL_COMPLETED
			&& !this.mIsCrazyDaveShowing) {

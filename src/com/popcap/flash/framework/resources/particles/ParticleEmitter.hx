...
class ParticleEmitter {
	...
	function spawnParticle(index:Int, spawnCount:Int):Particle {
		...
		var aPosX:Float = 0;
		var aPosY:Float = 0;
		if (aType == ParticleEmitterType.CIRCLE
			|| aType == ParticleEmitterType.CIRCLE_PATH
			|| aType == ParticleEmitterType.CIRCLE_EVEN_SPACING) {
	...
	function getRenderParams(particle:Particle, params:ParticleRenderParams):Bool {
		...
		if (this.getRenderParams(aCrossParticle, aCrossFadeParams)) {
			...
			params.mParticleScale = this.crossFadeLerp(aCrossFadeParams.mParticleScale, params.mParticleScale, aCrossFadeParams.mParticleScaleIsSet,
				params.mParticleScaleIsSet, aFraction);
			params.mParticleStretch = this.crossFadeLerp(aCrossFadeParams.mParticleStretch, params.mParticleStretch, aCrossFadeParams.mParticleStretchIsSet,
				params.mParticleStretchIsSet, aFraction);
			params.mSpinPosition = this.crossFadeLerp(aCrossFadeParams.mSpinPosition, params.mSpinPosition, aCrossFadeParams.mSpinPositionIsSet,
				params.mSpinPositionIsSet, aFraction);
	...
	function renderParticle(g:Graphics2D, particle:Particle, params:ParticleRenderParams) {
		...
		aImage.setColor(params.mAlphaIsSet ? params.mAlpha : 1, params.mRedIsSet ? params.mRed : 1, params.mGreenIsSet ? params.mGreen : 1,
			params.mBlueIsSet ? params.mBlue : 1);
	...
	function updateParticle(particle:Particle):Bool {
		...
		var aSpinSpeed = this.mEmitterDef.mParticleSpinSpeed.evaluate(particle.mParticleTimeValue,
			particle.mParticleInterp[Particle.TRACK_PARTICLE_SPIN_SPEED]) * SECONDS_PER_UPDATE;
		var aSpinAngle = this.mEmitterDef.mParticleSpinAngle.evaluate(particle.mParticleTimeValue,
			particle.mParticleInterp[Particle.TRACK_PARTICLE_SPIN_ANGLE]);
		var aLastSpinAngle = this.mEmitterDef.mParticleSpinAngle.evaluateLast(particle.mParticleLastTimeValue,
			particle.mParticleInterp[Particle.TRACK_PARTICLE_SPIN_ANGLE]);

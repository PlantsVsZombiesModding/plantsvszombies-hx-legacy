...
class Reanimation {
	...
	function drawTrackLerp(g:Graphics2D, track:ReanimTrack, alpha:Float) {
		...
		img.useColor = true;
		img.setColor(alpha * tA, this.overrideColor.red
			+ this.additiveColor.red, this.overrideColor.green
			+ this.additiveColor.green,
			this.overrideColor.blue
			+ this.additiveColor.blue);
		g.pushState();
		g.transform(matrix);
	...
	function drawTrack(g:Graphics2D, track:ReanimTrack) {
		...
		if (cache != null) {
			...
			if (cache.useColor) {
				cache.setColor(this.overrideColor.alpha
					+ this.additiveColor.alpha, this.overrideColor.red
					+ this.additiveColor.red,
					this.overrideColor.green
					+ this.additiveColor.green, this.overrideColor.blue
					+ this.additiveColor.blue);
			}
